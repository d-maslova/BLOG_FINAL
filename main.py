...
# CONNECT TO DB
app.config['SQLALCHEMY_DATABASE_URI'] = os.environ.get("DATABASE_URL", "sqlite:///blog.db")  # allows db to be accessed locally (sqlite) and remotely(DATABASE_URL -on heroku)
...
db.create_all()
# User.__table__.create(db.session.bind) # adds new table (User) to existing database (BlogPost)
